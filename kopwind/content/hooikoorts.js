/** content/hooikoorts.js: teksten van de hooikoortscheck. */

export const seo = {
  title: "Hooikoorts vandaag: pollen per uur en het rustigste blok",
  description:
    "Krijg ik vandaag hooikoorts? Zie gras-, berk- en elspollen per uur, stel je gevoeligheid in en vind het rustigste blok van de dag. Gratis.",
  h1: "Krijg ik vandaag hooikoorts?",
  intro:
    "Twee miljoen Nederlanders voelen aan hun neus wat voor dag het wordt. De check laat per uur zien hoeveel gras-, berk- en elspollen er in de lucht zitten, en wanneer het rustigste blok valt om te luchten, sporten of de was op te hangen.",
};

export const blokken = [
  {
    kop: "Drie soorten, drie seizoenen",
    tekst:
      "Els piekt als eerste (februari-maart), berk volgt in april en gras draagt de langste periode: mei tot en met augustus. De check toont per soort de klasse van vandaag, zodat je weet of jouw allergeen in de lucht zit. Reageer je maar op een van de drie? Stel dat in en de rest telt minder zwaar mee.",
  },
  {
    kop: "Het rustigste blok is het nieuwe beste blok",
    tekst:
      "Waar de terrascheck het lekkerste moment zoekt, zoekt deze check het rustigste: het langste blok met weinig pollen. Vaak is dat de vroege ochtend of vlak na een bui, want regen slaat pollen neer. Handig voor je hardlooprondje, het luchten van de slaapkamer en de was aan de lijn.",
  },
  {
    kop: "Eerlijk over de data",
    tekst:
      "De pollencijfers komen uit het Europese CAMS-model (Copernicus) op een raster van 11 kilometer, hetzelfde ensemble waar ook het KNMI aan bijdraagt. Goed voor het dagbeeld van jouw regio, niet voor het bloeiende grasveld om de hoek. De klassegrenzen per soort zijn vuistregels, geen medische standaard; met de gevoeligheidsinstelling schuif je ze naar jouw neus.",
  },
];

export const faq = [
  {
    v: "Wanneer is het hooikoortsseizoen?",
    a: "Grofweg februari tot en met augustus, in golven: els en hazelaar in de late winter, berk in april, gras van mei tot augustus. Buiten het seizoen zegt de check gewoon dat er niets in de lucht zit. Warme, droge en winderige dagen zijn de zwaarste; regen geeft tijdelijk lucht.",
  },
  {
    v: "Waarom is hooikoorts 's ochtends of 's avonds erger?",
    a: "Pollen worden overdag door opstijgende warme lucht omhoog gebracht en dalen in de avond weer neer. In de stad piekt de overlast daardoor vaak 's avonds, op het platteland juist in de ochtend als het gras bloeit. De urenstrip laat het verloop voor jouw plek zien.",
  },
  {
    v: "Helpt was binnen drogen tegen hooikoorts?",
    a: "Op pollendagen wel: buiten hangend wasgoed vangt pollen, die je daarna mee naar binnen neemt. De wascheck en deze check versterken elkaar dus: is het droogweer maar pollenpiek, overweeg dan het rek binnen. Zelfde verhaal voor het luchten van de slaapkamer: doe dat in het rustigste blok.",
  },
  {
    v: "Is deze hooikoortscheck medisch advies?",
    a: "Nee. De check vertaalt modeldata naar een praktisch dagbeeld en is geen vervanging van je huisarts of apotheker. Heb je veel last, begin dan tijdig met je medicatie (die werkt vaak beter preventief) en bespreek aanhoudende klachten met een professional.",
  },
];
